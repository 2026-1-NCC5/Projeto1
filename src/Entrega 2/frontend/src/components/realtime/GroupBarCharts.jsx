    ...
    <div className="realtime-grid fade-in" style={{ animationDelay: '0.1s', animationFillMode: 'both', marginBottom: '1.5rem' }}>
      ...
      <div className="chart-container">
        ...
        {appState.length === 0 ? (
          ...
          appState.map((group) => {
            const nItens = group.quantidadeItens ?? group.items?.length ?? 0;
            return (
            <div key={group.id} className="hbar-row">
              <div className="hbar-label">{group.title}</div>
              <div className="hbar-track">
                <div className="hbar-fill hbar-fill-alt" style={{ width: `${Math.max((nItens / maxGroupItems) * 100, 2)}%` }}>
                  {nItens > 0 && <span className="hbar-value">{nItens}</span>}
                </div>
              </div>
              <div className="hbar-side-value" style={{ color: '#3b82f6' }}>{nItens} itens</div>
            </div>
            );
          })
        )}
