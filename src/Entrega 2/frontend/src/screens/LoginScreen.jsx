...
export default function LoginScreen() {
  ...
  const handleSubmit = async (e) => {
    ...
    setEnviando(true);
    try {
      const r = await authLogin({ nome: nome.trim(), email: email.trim() });
      if (!r.ok) {
        const det = r.data?.detail;
        const msg = typeof det === 'string' ? det : 'Não foi possível entrar. Verifique nome e e-mail.';
        addToast(msg, 'error');
        return;
      }
      await refreshMe();
  ...
  return (
    <div className="flex flex-col h-full bg-dark">
      <header className="flex align-center p-6 border-b border-gray-medium bg-header shadow-md relative z-10">
        ...
        <span className="font-bold text-xl text-white tracking-tight">Login Administrador</span>
      </header>
      ...
      >
        ...
        >
          <div className="flex flex-col align-center gap-3 mb-8">
            <img
              src={LogoAbrace}
              alt="AbraceAI"
              style={{ width: '96px', height: '96px', objectFit: 'contain', clipPath: 'inset(0 0 18% 0)' }}
            />
            <h1 className="font-black text-2xl text-white text-center tracking-tight">
              ABRACE<span className="text-primary">AI</span>
            </h1>
            <p className="text-gray text-sm text-center">Nome completo e e-mail institucional cadastrados.</p>
          </div>

          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            <div className="flex flex-col gap-2">
              <label className="text-xs text-gray font-bold uppercase tracking-widest ml-1">Nome (como no cadastro)</label>
              <input
                type="text"
            ...
            <button type="submit" className="btn btn-primary w-full py-4 rounded-2xl shadow-lg" disabled={enviando}>
              <span className="font-bold">{enviando ? 'Entrando…' : 'Entrar'}</span>
          ...
          <div className="auth-screen-footer">
            <p className="text-sm text-gray">
              Não tem conta?{' '}
              <button type="button" className="auth-screen-footer-link" onClick={() => setCurrentScreen('cadastro')}>
                Cadastre uma conta
              </button>
            </p>
          </div>
