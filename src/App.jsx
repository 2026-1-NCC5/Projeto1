...
export default function App() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('abraceai_appState');
      if (saved) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('abraceai_appState', JSON.stringify(appState));
  ...
  useEffect(() => {
    ...
    try {
      const rt = window.io('http://localhost:5000', {
  ...
  const handleFinishManual = () => {
    if (manualGroupId && manualItems.length > 0) {
      setAppState(prev => prev.map(group => {
        if (group.id === manualGroupId) {
  ...
  if (currentScreen === 'realtime') return renderRealtimeScreen();

  // ===== MANUAL INSERTION SCREEN =====
  if (currentScreen === 'manual') {
    const manualGroup = appState.find(g => g.id === manualGroupId);
    return (
      <div className="flex flex-col h-full bg-dark">
        <header className="flex justify-between align-center p-5 border-b border-gray-medium bg-header shadow-md relative z-10">
          ...
          <div className="text-xs font-bold text-white bg-black/40 backdrop-blur px-3 py-1.5 rounded-full border border-gray-medium shadow flex align-center gap-2">
            ...
            GRUPO {manualGroup?.title || manualGroupId}
          </div>
        </header>
            ...
            {toasts.map(t => (
              <div key={t.id} className={`toast toast-${t.type} ${t.exiting ? 'toast-exit' : ''}`}>
                <div className="toast-icon">
                  <i className={`ph-fill ${t.type === 'success' ? 'ph-check-circle' : t.type === 'error' ? 'ph-x-circle' : t.type === 'warning' ? 'ph-warning' : 'ph-info'}`}></i>
                </div>
                <span>{t.message}</span>
              </div>
            ))}
          ...
          {appState.map(group => (
            <div key={group.id} className="kanban-card bg-card shadow-lg rounded-xl p-5 flex flex-col justify-between transition border-gray-medium" style={{ minHeight: '500px' }}>
              ...
              <div className="mt-auto pt-4 relative">
                ...
                <button className="btn btn-outline w-full mt-3 py-3 rounded-xl font-bold text-md border-gray-medium text-white hover-bg-gray-light" onClick={() => handleStartManual(group.id)}>
  ...
  // ===== REALTIME VISUALIZATION SCREEN =====
  function renderRealtimeScreen() {
    ...
    const maxProductKg = products.length > 0 ? products[0].totalKg : 1;

    // Recent items
    const recentItems = allItems.slice(-10).reverse();

    // Total members
    const totalMembers = appState.reduce((acc, g) => acc + g.members.length, 0);
